...
export default function DailyProfitCard({ changeAmount = 0, quantity, isLoading }: DailyProfitCardProps) {
    if (isLoading) {
        return (
        ...
        >
            <Card className="bg-gradient-to-br from-slate-900 to-slate-950 border-slate-800 text-white overflow-hidden relative h-full flex flex-col justify-center">
                ...
                <CardContent className="px-6 py-0">
                    <div>
                        <p className="text-slate-400 text-xs font-bold uppercase mb-4">일간 수익</p>
                        <div className="flex items-baseline gap-2 pt-4">
                            <h2 className={`text-2xl font-black tracking-tighter transition-all duration-300 ${
                                isPositive ? 'text-red-400' : isNegative ? 'text-blue-400' : 'text-slate-400'
                            }`}>
                                <span className="inline-block scale-y-[1.5] origin-bottom leading-none">
                                    {isPositive ? '+' : ''}{dailyProfit.toLocaleString()}
                                </span>
                            </h2>
