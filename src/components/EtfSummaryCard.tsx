            ...
            <Card className="bg-gradient-to-br from-slate-900 to-slate-950 border-slate-800 text-white overflow-hidden relative h-full flex flex-col justify-center">
                ...
                <CardContent className="px-6 py-0 flex-1 flex flex-col justify-center">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-slate-400 text-xs font-bold uppercase mb-4">{title}</p>
                            <h2 className="text-4xl font-bold tracking-tight">
                                {data.price.toLocaleString()}
                                <span className="text-slate-500 text-lg ml-2 font-normal">KRW</span>
                            </h2>
                        </div>
                        <div className={`flex items-center gap-2 px-3 py-1 rounded-full text-sm font-bold mt-1 ${isPositive ? 'bg-red-500/10 text-red-400' : isNegative ? 'bg-blue-500/10 text-blue-400' : 'bg-slate-500/10 text-slate-400'
                            }`}>
