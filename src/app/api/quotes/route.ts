...
const simulatedVolumes: Record<string, number> = {};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // 1. Fetch Indices
    const fetchIndices = axios
      .get(
        "https://stock.naver.com/api/polling/domestic/index?itemCodes=KOSPI,KOSDAQ,KPI200,KOSDAQ150",
    ...
    for (let i = 0; i < allCodesForPolling.length; i += batchSize) {
      ...
      stockBatches.push(
        axios.get(url, { headers: NAVER_HEADERS }).catch((err) => {
          console.error(
            `Stock Batch ${i} Fetch Error:`,
            err.response?.status,
            url,
          );
          return { data: { datas: [] } };
    ...
    const [indexRes, indicatorRes, ...batchResults] = await Promise.all([
      fetchIndices,
      fetchIndicators,
      ...stockBatches,
    ]);
    ...
    const kosdaq150Data = indexDatas.find(
      (d: any) => d.itemCode === "KOSDAQ150",
    );
    ...
    const usdKrwData = Array.isArray(indicatorDatas)
      ? indicatorDatas.find((d: any) => d.itemCode === "FX_USDKRW")
      : null;
    ...
    const quotes: Quote[] = components.map((stock) => {
      ...
      let price =
        Number(naverData?.closePriceRaw) || parsePrice(stock["현재가(원)"]);
      let changeAmount =
        Number(naverData?.compareToPreviousClosePriceRaw) ||
        parseChange(stock["등락(원)"]);
      ...
      if (isLiveMode && marketOpen) {
        ...
        if (!simulatedVolumes[stock.종목코드])
          simulatedVolumes[stock.종목코드] = volume;
        ...
        const prevPrice =
          (Number(naverData?.closePriceRaw) || price) -
          (Number(naverData?.compareToPreviousClosePriceRaw) || changeAmount);
        changeAmount = price - prevPrice;
        changeRate =
          prevPrice !== 0 ? (changeAmount / prevPrice) * 100 : changeRate;
    ...
    let etfPrice =
      Number(etfData?.closePriceRaw) || Math.round(kpi200ValueRaw * 100);
    ...
    if (isLiveMode && marketOpen) {
      ...
      etfChangeRate =
        prevPrice !== 0 ? (etfChangeAmount / prevPrice) * 100 : etfChangeRate;
    }
